...
## [2026-09-01] task | докачка data/full/clubs.json до скачиваемых лиг (topup.py)

Новый скрипт 	opup.py: для клубов из data/clubs.json (4574), входящих хотя бы в одну из
266 скачиваемых лиг 	ournament_structure.json и отсутствующих в data/full/clubs.json,
качает clubs/{id}/profile + clubs/{id}/players (2 запроса/клуб, HTML 2 rps). Профиль даёт
авторитетную страну/лигу — вмердж в существующую вложенную структуру без эвристик, атомарный
флаш каждые 50 клубов. Resume — SQLite-кэш (провалы не кэшируются).

Структура расширена на 28 лиг: 24 страны-топ-лиги (Алжир, Египет, ОАЭ и др.), PLI3 (Перу, 3-й
тир), UA2P (Украина, youth U21), UAE 2 тира; NZNL (чемпионский плей-офф НЗ) — НЕ скачиваем.
UA2P оказалась молодёжной лигой U21 (клубы типа «Динамо Киев U21») — включена в youth по
решению пользователя.

Качка картинок — единая фаза полного скана файла (логотипы/флаги/лица), CDN-бакет 15 rps
(адаптив), one-shot (max_retries=1, не открывает circuit), mirror-fallback на tmssl
(img.a флапает 502/504), плейсхолдеры default и существующие файлы пропускаются,
прогресс-лог каждые 2000 лиц.

Итог: countries 65→106, leagues 94→279, clubs 1598→4406, players 44,933→121,473,
faces 28,634→66,486, logos 1598→4402, league logos 98→280, flags 65→106.
Пропущен 1 клуб (67241 «Кубань Краснодар» — профиль без league, состав 1 игрок).

Известный рассинхрон: структура использует «игровые» id лиг (ARG1, MEX1, TDeA), клубы в файле
лежат под реальными TM-id (ARGC, MEXA, TDeC) — конвертеру нужен маппинг.
