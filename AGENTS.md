...
- **Лимиты к TM: HTML 8 rps, CDN 15 rps** (установлены probe 2026-08-29, см. [[конвейер]]).
  Скорость самонастраивается: троттлинг делит rps пополам (пол 1), успехи плавно растят обратно;
  выученная скорость сохраняется в `data/rates.json` и используется следующей сессией.
  При бане (429/403/блок-страница) — не молотить ретраями; circuit breaker даст паузу,
  заблокированные URL кэшируются.
