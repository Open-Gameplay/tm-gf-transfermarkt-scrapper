...
- **Лимиты к TM: HTML 2 rps, CDN 15 rps** (установлены probe 2026-08-29, см. [[конвейер]]).
  Скорость самонастраивается: троттлинг/серия 403 делит rps пополам (пол 0.5), успехи плавно
  растят обратно; выученная скорость сохраняется в `data/rates.json`.
  При бане — не молотить ретраями: circuit breaker даст паузу, адаптивная скорость снизится,
  провалы игроков дотягивает retry-очередь (не кэшируются).
